...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'Identifiant du modèle manquant' }, { status: 400 })
    if (!name) return NextResponse.json({ error: 'Nom du modèle manquant' }, { status: 400 })
    ...
    // Si c'est un modèle en ligne (HTML)
    if (htmlContent !== null) {
    ...
    if (file) {
      ...
      if (existing.storageName) {
        await supabase.storage.from('crm-attachments').remove([`templates/${existing.storageName}`])
      }
      ...
      // Just updating metadata of the Word file
      const template = await prisma.documentTemplate.update({
        where: { id },
        data: {
          name,
